...
class AlphabeticalComparatorTest {
	...
	public void ChildrenHomeDiretoryComparatorTest() {
		...
		String[] actual = apfsEleToString((LinkedList<apfsElement>) root.findDirectory("home").getChildren(new AlphabeticalComparator()));
	...
	public void FileInHomeDirectoryComparatorTest() {
		...
		apfs FilesystemApfs = apfs.getAPFSFileSystem();
		apfsDirectory root = (apfsDirectory)FilesystemApfs.getRoot();
		String[] actual = apfsFileToString((LinkedList<apfsFile>) root.findDirectory("home").getFiles(new AlphabeticalComparator()));
		assertArrayEquals(expected, actual);
	}
